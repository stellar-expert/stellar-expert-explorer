...
export const AccountTrustlineBalanceView = React.memo(
    function AccountTrustlineBalanceView({trustline, currency, onClick}) {
        ...
        return <a href="#" className={cn('account-balance', {deleted: trustline.deleted})} onClick={onBalanceClick}
                  ref={root}>
        ...
        <span className="text-small">
            ...
            {((trustline.flags & 1) !== 1 && trustline.asset) &&
                <i className="icon icon-lock"
                   title={`Trustline to ${(asset).split('-')[0]} is not authorized by the asset issuer`}/>}
        </span>
