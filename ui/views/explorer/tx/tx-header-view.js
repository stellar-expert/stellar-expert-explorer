...
export default function TxHeader({tx, embedded}) {
    return React.createElement(embedded ? 'h4' : 'h2', {className: 'word-break relative'},
        embedded ? <a href={resolvePath(`tx/${tx.id}`)}>
                Transaction <BlockSelect wrap inline>{tx.id}</BlockSelect></a> :
            <span>Transaction <BlockSelect wrap inline>{tx.id}</BlockSelect></span>,
        ...
        <Tracer endpoint={`transactions/${tx.id}`}/>,
        <EmbedWidgetTrigger path={`tx/info/${tx.id}`} title="Transaction Details"/>)
}